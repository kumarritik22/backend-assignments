...
const ForgotPassword = () => {
    ...
    const renderForgotPasswordCard = () => {
        ...
        return <div className="loading-state w-full max-w-md bg-[#111111] border border-[#2A2A2A] rounded-lg shadow-2xl flex items-center flex-col justify-center p-12">
            ...
            <h2 className="text-2xl font-medium tracking-tight text-center mb-2">Sending Reset Link...</h2>
            <p className="font-sans text-[#888888] text-sm leading-relaxed text-center mb-7">Please wait while we prepare your secure password reset link.</p>
        </div>
        } else if (error) {
        return <div className="error-state w-full max-w-md bg-[#111111] border border-[#2A2A2A]    rounded-lg shadow-2xl flex items-center flex-col justify-center p-12">
            ...
            <h2 className="text-2xl font-medium tracking-tight text-center mb-2">Unable to Send Reset Link</h2>
        ...
        return <div className="success-state w-full max-w-md bg-[#111111] border border-[#2A2A2A]     rounded-lg shadow-2xl flex items-center flex-col justify-center p-12">
            ...
            {authType === "emailSent" && (
                <p className="font-sans text-[#888888] text-sm leading-relaxed text-center mb-7">We have sent a password reset link to your email address. Please check your inbox and spam folder.</p>
            )}
            {authType === "googleAccount" && (
                <p className="font-sans text-[#888888] text-sm leading-relaxed text-center mb-7">This account is registered with Google. Please continue with Google to access your account.</p>
            )}
            {authType === "emailSent" ? (
                <Link 
                    to="/login"
