...
const MyOrders = () => {
    ...
    if (isLoading) {
      return (
        <div className="min-h-screen bg-[#F6F5F2] dark:bg-[#0a0a0a] flex flex-col items-center justify-center">
    ...
    if (!orders || orders.length === 0) {
      return (
        <div className="min-h-screen bg-[#F6F5F2] dark:bg-[#0a0a0a] text-[#121212] dark:text-white flex flex-col items-center justify-center px-4 pt-10 pb-20">
          <div className="bg-white dark:bg-[#121212] border border-black/5 dark:border-white/10 rounded-2xl p-8 sm:p-12 max-w-md w-full text-center space-y-5 shadow-xl dark:shadow-2xl">
            ...
            <h2 className="font-bodoni text-2xl font-bold text-[#121212] dark:text-white">Your Order Vault is Empty</h2>
            <p className="font-inter text-xs text-[#636059] dark:text-[#888] leading-relaxed">
  ...
  return (
    <div className="min-h-screen bg-[#F6F5F2] dark:bg-[#0a0a0a] text-[#121212] dark:text-white pt-6 sm:pt-8 pb-20 px-4 sm:px-6 lg:px-12 selection:bg-gold/30">
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-black/5 dark:border-white/5 pb-6">
          <div className="space-y-1">
            ...
            <p className="font-inter text-xs text-[#636059] dark:text-[#888]">
              Review your past orders, delivery tracking, and bespoke receipts.
            </p>
          </div>
          <span className="font-inter text-xs text-[#636059] dark:text-[#888] uppercase tracking-wider">
        ...
        <div className="space-y-6">
          {orders.map((order) => {
            ...
            return (
              <div 
                key={order._id}
                className="bg-white dark:bg-[#121212]/90 border border-black/5 dark:border-white/10 hover:border-gold/40 dark:hover:border-gold/30 rounded-2xl p-6 sm:p-8 backdrop-blur-xl shadow-md dark:shadow-2xl transition-all duration-300 space-y-6 group"
              >
                {/* Card Header Strip */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-black/5 dark:border-white/5">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-xs text-[#888] dark:text-[#666]">·</span>
                    <span className="font-inter text-xs text-[#636059] dark:text-[#888]">
                  ...
                  {order.orderItems?.map((item, idx) => (
                    <div key={idx} className="py-4 first:pt-0 last:pb-0 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-16 sm:w-16 sm:h-20 rounded-lg overflow-hidden bg-[#f0ede6] dark:bg-[#181818] shrink-0 border border-black/5 dark:border-white/10">
                          <img 
                        ...
                        <div>
                          <h4 className="font-bodoni text-base font-bold text-[#121212] dark:text-white group-hover:text-gold transition-colors">
                            {item.title}
                          </h4>
                          {item.attributes && Object.keys(item.attributes).length > 0 && (
                            <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                              {Object.entries(item.attributes).map(([key, val]) => (
                                <span key={key} className="text-[10px] font-inter text-[#636059] dark:text-[#aaa] capitalize">
                                  <span className="text-[#888] dark:text-[#666]">{key}: </span>
                                  <span className="text-[#121212] dark:text-white font-medium">{val}</span>
                                  <span className="text-[#ccc] dark:text-[#444] ml-1.5 last:hidden">·</span>
                                </span>
                              ))}
                            </div>
                          )}
                          <p className="font-inter text-[11px] text-[#888] dark:text-[#666] mt-0.5">
                            Qty: {item.quantity}
                          </p>
                        </div>
                      </div>

                      <p className="font-bodoni text-base font-bold text-[#121212] dark:text-white text-right">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-5 border-t border-black/5 dark:border-white/5">
                  <div className="flex items-baseline gap-2">
                    <span className="font-inter text-xs text-[#636059] dark:text-[#888] uppercase tracking-wider">Total Paid:</span>
